 ...
 *
 * @author auto create
 * @since 1.0, 2020-08-24 10:53:18
 */
public class KoubeiCateringOrderInfoSyncModel extends AlipayObject {

	private static final long serialVersionUID = 3316477498138985182L;
...
接单时action_info中加桌号(tableNo)，tableNo和no只能传一种，并确定取餐方式;修改桌号时action_info中传tableNo，thirdPayNo:外部订单号,estimatePickUpTime:预计取餐时间，格式为:${xx}-${yy},${xx}和${yy}为数字，单位为分钟
4、部分退款调用时，out_refund_no为外部退款单号来保证幂等，refund_amount为部分退款金额。5、排队顺序同步时，action_info中加排队序号queueNum。 ；拒绝修改预约时间原因:1、EXCEED_LIMIT 修改次数超限，2、ERROR_STATUS 当前状态不允许修改  ，3、TIME_INVALID 不是有效的预约时间，4、OTHER 其他原因。
	 */
