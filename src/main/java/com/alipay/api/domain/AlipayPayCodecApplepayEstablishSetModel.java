 ...
 *
 * @author auto create
 * @since 1.0, 2019-11-27 12:27:00
 */
public class AlipayPayCodecApplepayEstablishSetModel extends AlipayObject {

	private static final long serialVersionUID = 5719675163316364499L;
...
barcodeEncryptionCertificate , deviceSignatureCertificate .
	...
	/**
	 * JSON格式，base64的证书链List。两级证书结构，能被root证书验过。第0个证书用于二维码加密使用。
Base 64 encoded X.509 encryption certificate chain containing the intermediate and leaf certificates. Encryption shall be performed using the first certificate in the list.
	...
	/**
	 * 证书类型。
List of certificate types that are requested from Wallet. See Appendix B for list of all possible values. deviceSignatureCertChain:This certificate chain contains the intermediate and leaf certificates whose public key corresponds to the private key that device will use to generate the signature for the Timestamp API;barcodeEncryptionCertChain:This certificate contains the intermediate and leaf certificates whose public key the Partner will use to.
	 */
	@ApiListField("certificates_requested")
	@ApiField("string")
	private List<String> certificatesRequested;

	/**
	 * JSON格式，base64的证书链List。两级证书结构，能被root证书验过。第0个证书用于二维码上报加签使用。
...
The identifier for which the feature is being enabled.
	...
	public List<String> getCertificatesRequested() {
		return this.certificatesRequested;
	}
	public void setCertificatesRequested(List<String> certificatesRequested) {
		this.certificatesRequested = certificatesRequested;
	}
