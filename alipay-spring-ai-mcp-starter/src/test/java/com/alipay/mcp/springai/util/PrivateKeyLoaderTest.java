...
class PrivateKeyLoaderTest {
    ...
    void testLoadFromStringWithWhitespace() {
        ...
        // 先清理出纯 base64，再格式化为换行版本（模拟真实 PEM 格式）
        String cleanBase64 = TEST_PRIVATE_KEY
            .replace("-----BEGIN PRIVATE KEY-----", "")
            .replace("-----END PRIVATE KEY-----", "")
            .replaceAll("\\s", "");

        // 每 64 个字符加一个换行（标准 PEM 格式）
        StringBuilder formatted = new StringBuilder("-----BEGIN PRIVATE KEY-----\n");
        for (int i = 0; i < cleanBase64.length(); i += 64) {
            formatted.append(cleanBase64, i, Math.min(i + 64, cleanBase64.length()));
            formatted.append("\n");
        }
        formatted.append("-----END PRIVATE KEY-----");

        String keyWithWhitespace = formatted.toString();
