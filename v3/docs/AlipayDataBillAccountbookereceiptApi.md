...
<a name="query"></a>
# **query**
> AlipayDataBillAccountbookereceiptQueryResponseModel query(fileId, agreementNo, agreementType, secure)
...
public class Example {
  public static void main(String[] args) {
    ...
    String secure = "true"; // String | true表示使用安全链接，即返回的下载链接为https。 非true值的情况下，统一为false，使用http链接。
    try {
      AlipayDataBillAccountbookereceiptQueryResponseModel result = apiInstance.query(fileId, agreementNo, agreementType, secure);
      System.out.println(result);
    } catch (ApiException e) {
